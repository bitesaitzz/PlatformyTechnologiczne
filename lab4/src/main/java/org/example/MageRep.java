...
public class MageRep {
    ...
    public List <Mage> showAll(){
        return em.createQuery("from Mage").getResultList();
    }
    public Optional<Mage> findName(String name){
        Mage mage = em.createQuery("Select b FROM Mage b where b.name = :name", Mage.class ).setParameter("name", name).getSingleResult();
